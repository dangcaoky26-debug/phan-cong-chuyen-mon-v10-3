# -*- coding: utf-8 -*-
"""
PHẦN MỀM PHÂN CÔNG CHUYÊN MÔN V10.3.8
Mô hình chính:
...
INDEX_PATH = os.path.join(RESOURCE_DIR, "index_v10_3_8.html")
TKB_TEMPLATE_PATH = os.path.join(RESOURCE_DIR, "MAU_NHAP_TKB_EXCEL_V10_3.xlsx")
HOST = os.environ.get("HOST", "0.0.0.0")
PORT = int(os.environ.get("PORT", "8838"))
PORT_FALLBACK_MAX = PORT if os.environ.get("PORT") else 8848
...
class Store:
    ...
    def setup(self):
        with self.lock:
            ...
            c.execute("""CREATE TABLE IF NOT EXISTS assignment_changes(
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                year TEXT NOT NULL,
                ...
                subject TEXT NOT NULL,
                req_type TEXT NOT NULL,
                from_teacher TEXT DEFAULT '',
                to_teacher TEXT DEFAULT '',
                start_week INTEGER DEFAULT 1,
                end_week INTEGER DEFAULT 35,
                note TEXT DEFAULT '',
                created_at TEXT DEFAULT '',
                UNIQUE(year,requirement_id,start_week)
            )""")
            ...
            # V10.3.3: Trung tâm GDTX dùng hệ số 1,8 giáo viên/lớp.
            self.ensure_gdtx_teacher_per_class()

    def ensure_gdtx_teacher_per_class(self):
        """V10.3.3: sửa hệ số nhân sự GDTX từ giá trị THPT cũ 2,25 về 1,8 GV/lớp.

        Chỉ tự động thay khi DB chưa có giá trị hoặc vẫn còn đúng giá trị cũ 2,25;
        nếu đơn vị đã cấu hình một giá trị khác thì tôn trọng cấu hình đó.
        """
        with self.lock:
            row=self.cx.execute("SELECT value FROM settings WHERE key='teacher_per_class'").fetchone()
            current=None
            if row:
                try: current=json.loads(row["value"])
                except Exception: current=row["value"]
            try: current_num=float(str(current).replace(",",".")) if current not in (None,"") else None
            except Exception: current_num=None
            if current_num is None or abs(current_num-2.25)<1e-9:
                self.cx.execute("INSERT OR REPLACE INTO settings(key,value) VALUES('teacher_per_class',?)",
                                (json.dumps(1.8,ensure_ascii=False),))

            # TT 20/2023 áp cho cơ sở giáo dục phổ thông; không dùng nhãn đó làm nguồn
            # cho hệ số nhân sự của trung tâm GDTX trong phần mềm này.
            src_row=self.cx.execute("SELECT value FROM settings WHERE key='sources'").fetchone()
            if src_row:
                try: sources=json.loads(src_row["value"] or "{}")
                except Exception: sources={}
                if isinstance(sources,dict) and sources.get("staffing") in ("TT 20/2023/TT-BGDĐT","Thông tư 20/2023/TT-BGDĐT"):
                    sources["staffing"]="Định mức GDTX: 1,8 GV/lớp"
                    self.cx.execute("INSERT OR REPLACE INTO settings(key,value) VALUES('sources',?)",
    ...
    def workload_policy(self,year=None):
        """Quy tắc tính tải theo năm học, không làm thay đổi dữ liệu phân công gốc."""
        year=year or self.active_year()
        raw=self.setting(f"workload_policy::{year}",{}) or {}
        weeks=int(self.setting("weeks",35))
        exam=[]
        for w in raw.get("exam_weeks",[18,35]):
            try:
                iw=int(w)
                if 1<=iw<=weeks:exam.append(iw)
            except Exception:
                pass
        exam=sorted(set(exam)) or [w for w in (18,35) if w<=weeks]
        return {
            "exam_neutral_enabled":bool(raw.get("exam_neutral_enabled",False)),
            "exam_weeks":exam,
            "exam_week_standard":max(0.0,num(raw.get("exam_week_standard",17),17)),
            "annual_overtime_limit":max(0.0,num(raw.get("annual_overtime_limit",200),200))
        }

    def save_workload_policy(self,payload,year=None):
        year=year or self.active_year();weeks=int(self.setting("weeks",35))
        raw_weeks=payload.get("exam_weeks",[18,35])
        if isinstance(raw_weeks,str):
            raw_weeks=re.split(r"[,;\s]+",raw_weeks.strip()) if raw_weeks.strip() else []
        exam=[]
        for w in raw_weeks or []:
            try:
                iw=int(w)
                if 1<=iw<=weeks:exam.append(iw)
            except Exception:
                pass
        policy={
            "exam_neutral_enabled":bool(payload.get("exam_neutral_enabled",False)),
            "exam_weeks":sorted(set(exam)),
            "exam_week_standard":max(0.0,num(payload.get("exam_week_standard",17),17)),
            "annual_overtime_limit":max(0.0,num(payload.get("annual_overtime_limit",200),200))
        }
        self.set_setting(f"workload_policy::{year}",policy)
        self.invalidate_caches()
        return self.workload_policy(year)

    def is_neutral_exam_week(self,week,year=None):
        p=self.workload_policy(year)
        return p["exam_neutral_enabled"] and int(week) in set(p["exam_weeks"])
    ...
    def full_backup(self):
        tables=["academic_years","settings","teachers","templates","template_items","classes",
                "program_rules","requirements","assignment_changes","review_jobs","duties","locks","audit_log","scenarios",
    ...
    def restore_full_backup(self,backup):
        ...
        delete_order=["workflow_log","audit_log","scenarios","timetable_entries","staff_events","locks",
                      "duties","review_jobs","assignment_changes","requirements","classes","template_items","templates","teachers",
    ...
    def full_snapshot(self,year=None):
        ...
        with self.lock:
            ...
            return {
                ...
                "assignment_changes":[dict(r) for r in self.cx.execute("SELECT * FROM assignment_changes WHERE year=?",(year,))],
    ...
    def restore_snapshot(self,snapshot):
        ...
        with self.lock:
            ...
            for table in ["assignment_changes","requirements","review_jobs","duties","locks","staff_events","timetable_entries","classes","teachers","templates"]:
            ...
            self._insert_rows("assignment_changes",snapshot.get("assignment_changes",[]))
    ...
    def plan_metrics(self):
        ...
        annual_over_limit_count=0;sum_annual_overtime=0;max_annual_overtime=0
        max_loads=[]
        for t in teachers:
            ...
            annual=self.teacher_annual_summary(t["name"],year)
            aover=num(annual.get("over_hours"));sum_annual_overtime+=aover;max_annual_overtime=max(max_annual_overtime,aover)
            if annual.get("over_limit"):annual_over_limit_count+=1
        ...
        return {
            ...
            "sum_over":sum_over,
            "max_over":max_over,
            "annual_over_limit_count":annual_over_limit_count,
            "sum_annual_overtime":sum_annual_overtime,
            "max_annual_overtime":max_annual_overtime,
            "annual_overtime_limit":self.workload_policy(year)["annual_overtime_limit"],
            "wrong_subject":wrong,
    ...
    def recommend_teachers(self,class_name,subject,req_type=None,week=1,limit=10):
        ...
        for t in self.teachers(active_only=True):
            ...
            sw=max(int(week or 1),min(int(x["start_week"]) for x in parts));ew=max(int(x["end_week"]) for x in parts)
            ...
            if p.get("annual_after",{}).get("over_limit"):score-=250000
            ...
            rows.append({
                ...
                "annual_after":p.get("annual_after",{}),"score":score
            })
    ...
    def validate_all(self):
        ...
        # 1. Nhu cầu chưa có GV / sai chuyên môn / ngoài hiệu lực, xét đúng từng giai đoạn phân công.
        for r in reqs:
            ref=f'{r["class_name"]} · {r["subject"]} · {r["req_type"]}'
            for seg in self.assignment_segments_for_requirement(r,year):
                teacher=(seg.get("teacher") or "").strip();a=int(seg["start_week"]);b=int(seg["end_week"])
                if not teacher:
                    add("error","UNASSIGNED","Chưa phân công giáo viên",
                        f'{ref} chưa có giáo viên trong giai đoạn T{a}-{b}.',ref)
                    continue
                t=tmap.get(teacher)
                if not t:
                    add("error","TEACHER_MISSING","Giáo viên không tồn tại",
                        f'{ref} T{a}-{b} đang gán cho "{teacher}" nhưng giáo viên không có trong đội ngũ.',ref)
                    continue
                if r["subject"] not in t.get("can_teach_list",[]) and r["subject"]!=t.get("primary_subject"):
                    add("error","WRONG_SUBJECT","Không đúng chuyên môn",
                        f'{teacher} dạy {r["subject"]} ở {r["class_name"]} T{a}-{b} nhưng môn này không nằm trong thẻ chuyên môn.',teacher)
                if int(t["active_start"])>a or int(t["active_end"])<b:
                    add("warning","TEACHER_PERIOD","Hiệu lực giáo viên không phủ nhiệm vụ",
                        f'{teacher} hiệu lực T{t["active_start"]}-{t["active_end"]}, nhưng {ref} do giáo viên này phụ trách T{a}-{b}.',teacher)
        ...
        for t in self.teachers(year,active_only=True):
            ...
            annual=self.teacher_annual_summary(t["name"],year)
            limit=num(annual.get("annual_overtime_limit"),200)
            if annual.get("over_limit"):
                add("warning","ANNUAL_OVERTIME_LIMIT","Vượt ngưỡng dư giờ cả năm",
                    f'{t["name"]} có tổng dư cộng dồn {self._fmt_hours_vi(annual["over_hours"])} tiết/năm, vượt ngưỡng {self._fmt_hours_vi(limit)} tiết {self._fmt_hours_vi(annual["over_hours"]-limit)} tiết.',t["name"])
        ...
        for r in self.review_jobs(year):
            ...
            check_start=int(r["start_week"] or 1);check_end=int(r.get("end_week") or self.setting("weeks",35));changes_by=self._assignment_changes_by_req(year);eligible=set()
            for rr in reqs:
                if rr["subject"]!=r["subject"] or not str(rr["class_name"]).startswith("12"):continue
                a=max(check_start,int(rr.get("start_week",1)));b=min(check_end,int(rr.get("end_week",self.setting("weeks",35))))
                if a<=b and all(self.effective_teacher_for_requirement(rr,w,changes_by)==r["teacher"] for w in range(a,b+1)):
                    eligible.add(rr["class_name"])
        ...
        # 9. Biến động nhân sự làm gián đoạn phân công, xét phân công có hiệu lực theo tuần.
        changes_by=self._assignment_changes_by_req(year)
        for ev in self.staff_events(year=year):
            if not ev["blocks_teaching"]: continue
            impacted=[]
            for r in reqs:
                a=max(int(r["start_week"]),int(ev["start_week"]));b=min(int(r["end_week"]),int(ev["end_week"]))
                if a>b:continue
                if any(self.effective_teacher_for_requirement(r,w,changes_by)==ev["teacher"] for w in range(a,b+1)):
                    impacted.append(r)
            if impacted:
    ...
    def validate_timetable(self):
        ...
        # TKB vs phân công chuyên môn: xét đúng giáo viên có hiệu lực theo từng tuần.
        reqs=self.requirements();changes_by=self._assignment_changes_by_req(self.active_year())
        by_cs={}
        for r in reqs:by_cs.setdefault((r["class_name"],canonical_subject_name(r["subject"])),[]).append(r)
        ...
        for e in rows:
            ...
            key=(e["class_name"],canonical_subject_name(e["subject"]));parts=by_cs.get(key,[])
            a=max(1,int(e.get("start_week",1)));b=min(int(self.setting("weeks",35)),int(e.get("end_week",self.setting("weeks",35))))
            matches=any(any(self.effective_teacher_for_requirement(r,w,changes_by)==e["teacher"]
                            for r in parts if int(r.get("start_week",1))<=w<=int(r.get("end_week",self.setting("weeks",35))))
                        for w in range(a,b+1)) if parts else False
            k=(e["class_name"],canonical_subject_name(e["subject"]),e["teacher"])
            if not _is_tkb_admin_subject(e["subject"]) and not matches and k not in mismatch_seen:
                mismatch_seen.add(k)
                add("warning","ASSIGNMENT_MISMATCH",f'{e["teacher"]} có TKB {e["class_name"]} {e["subject"]} nhưng không khớp phân công có hiệu lực trong giai đoạn T{a}-{b}.')
    ...
    # ---------- Phân tích tác động ----------
    def impact_analysis(self,class_name,subject,teacher,req_type=None,week=1,effective_week=None,effective_end_week=None):
        parts=self._target_parts(class_name,subject,req_type)
        if not parts:raise ValueError("Không tìm thấy thẻ môn.")
        weeks=int(self.setting("weeks",35));effective_week=max(1,min(weeks,int(effective_week or week or 1)))
        effective_end_week=max(effective_week,min(weeks,int(effective_end_week or max(int(x.get("end_week",weeks)) for x in parts))))
        p=self.preview_drop(class_name,subject,teacher,req_type,week,effective_week,effective_end_week)
        changes_by=self._assignment_changes_by_req(self.active_year())
        old_teachers=sorted({self.effective_teacher_for_requirement(x,effective_week,changes_by)
                             for x in parts
                             if self.effective_teacher_for_requirement(x,effective_week,changes_by)
                             and self.effective_teacher_for_requirement(x,effective_week,changes_by)!=teacher})
        old_impacts=[]
        for ot in old_teachers:
            base=self.workload_matrix(self.active_year()).get(ot)
            sim=self.simulate_assignment_loads(ot,parts,effective_week,effective_end_week,"remove",self.active_year())
            ss=self._load_array_summary(sim,self.active_year())
            before=dict(base[int(week)]) if base and base[int(week)] is not None else None
            after=dict(sim[int(week)]) if sim and sim[int(week)] is not None else None
            annual_before=self._load_array_summary(base,self.active_year())["annual"] if base else {}
            old_impacts.append({"teacher":ot,"before":before,"after":after,"max_after":ss["max"],
                                "annual_before":annual_before,"annual_after":ss["annual"]})
        events=self.blocking_events(teacher,effective_week,effective_end_week)
        tkb_conflicts=self.timetable_conflicts_for_teacher(teacher,class_name,subject,effective_week,effective_end_week)
        return {"target":p,"old_teachers":old_impacts,"blocking_events":events,
                "timetable_conflicts":tkb_conflicts,"effective_week":effective_week,"effective_end_week":effective_end_week,
                "safe":bool(p["compatible"] and p["covers"] and not events and not tkb_conflicts and num(p["max_after"]["diff"])<=0
                            and not p.get("annual_after",{}).get("over_limit"))}
    ...
    def assistant_query(self,text,week=1):
        ...
        # Ai vượt/dư cả năm trên X tiết (đặc biệt ngưỡng 200)
        m_year=re.search(r"(dư|vượt).{0,18}(\d+(?:[,.]\d+)?)",ql)
        if ("năm" in ql or "cộng dồn" in ql or "200" in ql) and ("ai" in ql or "giáo viên" in ql) and m_year:
            thr=num(m_year.group(2));rows=[]
            for t in self.teachers(active_only=True):
                a=self.teacher_annual_summary(t["name"])
                if num(a.get("over_hours"))>=thr:rows.append((t["name"],num(a.get("over_hours")),num(a.get("annual_overtime_limit"))))
            rows.sort(key=lambda x:-x[1])
            ans="; ".join(f'{n}: dư cộng dồn {self._fmt_hours_vi(o)} tiết/năm'+(f' (vượt ngưỡng {self._fmt_hours_vi(l)})' if o>l else '') for n,o,l in rows)
            return {"answer":ans or "Không có giáo viên đạt ngưỡng dư giờ cả năm này.","actions":[]}
        # Ai dư/thiếu trên X tiết ở tuần đang xem
        ...
        for t in self.teachers(active_only=True):
            if t["name"].lower() in ql or t["name"].split()[-1].lower() in ql:
                d=self.teacher_load_at_week(t["name"],week);mx=self.teacher_max_load(t["name"]);annual=self.teacher_annual_summary(t["name"])
                return {"answer":f'{t["name"]}: tuần {week} {self._fmt_hours_vi(d["total"])}/{self._fmt_hours_vi(d["standard"])} tiết, dư/thiếu {self._fmt_hours_vi(d["diff"])}. Cao nhất năm T{mx["week"]}: {self._fmt_hours_vi(mx["total"])}/{self._fmt_hours_vi(mx["standard"])}. Dư cộng dồn năm {self._fmt_hours_vi(annual["over_hours"])}/{self._fmt_hours_vi(annual["annual_overtime_limit"])} tiết.',
                        "actions":[]}
    ...
    def workload_matrix(self,year=None):
        ...
        if cached is not None and cached[0]==stamp:
            return cached[1]
        weeks=int(self.setting("weeks",35))
        policy=self.workload_policy(year)
        neutral_exam_weeks=set(policy["exam_weeks"]) if policy["exam_neutral_enabled"] else set()
        exam_week_standard=num(policy.get("exam_week_standard"),17)
        rules={r["subject"]:r for r in self.program_rules()}
        teachers=self.teachers(year)
        with self.lock:
            reqs=[dict(r) for r in self.cx.execute("SELECT * FROM requirements WHERE year=?",(year,))]
            changes=[dict(r) for r in self.cx.execute("SELECT * FROM assignment_changes WHERE year=? ORDER BY start_week,id",(year,))]
        ...
        changes_by={}
        for x in changes:changes_by.setdefault(int(x["requirement_id"]),[]).append(x)
        req_by_week={}
        for r in reqs:
            for w in range(max(1,int(r.get("start_week",1))),min(weeks,int(r.get("end_week",weeks)))+1):
                teacher=self.effective_teacher_for_requirement(r,w,changes_by)
                if teacher:req_by_week.setdefault((teacher,w),[]).append(r)
        rev_by={};duty_by={};event_by={}
        ...
        for t in teachers:
            ...
            trev=rev_by.get(name,[]);tdut=duty_by.get(name,[]);tevents=event_by.get(name,[])
            for w in range(1,weeks+1):
                teaching=sum(self.requirement_weekly_hours(r,w,rules) for r in req_by_week.get((name,w),[]))
                review=0.0
                for r in trev:
                    sw=r.get("start_week")
                ...
                actual_total=teaching+review+duty_credit
                exam_neutral=(w in neutral_exam_weeks and not unavailable
                              and int(t.get("active_start",1))<=w<=int(t.get("active_end",weeks)) and int(t.get("active",1)))
                if exam_neutral:std=exam_week_standard
                total=std if exam_neutral else actual_total
                arr[w]={"teaching":teaching,"review":review,"role":role,"homeroom":homeroom,
                        ...
                        "staff_events":unavailable,"actual_total":actual_total,"exam_neutral":exam_neutral,
    ...
    def assignment_changes(self,year=None,requirement_id=None):
        year=year or self.active_year()
        q="SELECT * FROM assignment_changes WHERE year=?";params=[year]
        if requirement_id is not None:
            q+=" AND requirement_id=?";params.append(int(requirement_id))
        q+=" ORDER BY start_week,id"
        with self.lock:
            return [dict(r) for r in self.cx.execute(q,params)]

    def _assignment_changes_by_req(self,year=None):
        out={}
        for x in self.assignment_changes(year):
            out.setdefault(int(x["requirement_id"]),[]).append(x)
        return out

    def effective_teacher_for_requirement(self,req,week,changes_by_req=None):
        week=int(week);base=(req.get("teacher") or "").strip()
        if not (int(req.get("start_week",1))<=week<=int(req.get("end_week",self.setting("weeks",35)))):
            return ""
        if changes_by_req is None:
            changes_by_req=self._assignment_changes_by_req(req.get("year") or self.active_year())
        active=[x for x in changes_by_req.get(int(req["id"]),[])
                if int(x.get("start_week",1))<=week<=int(x.get("end_week",self.setting("weeks",35)))]
        if not active:return base
        latest=max(active,key=lambda x:(int(x.get("start_week",1)),int(x.get("id",0))))
        return (latest.get("to_teacher") or "").strip()

    def assignment_segments_for_requirement(self,req,year=None):
        year=year or req.get("year") or self.active_year()
        changes=self._assignment_changes_by_req(year)
        a=int(req.get("start_week",1));b=int(req.get("end_week",self.setting("weeks",35)))
        if a>b:return []
        vals=[(w,self.effective_teacher_for_requirement(req,w,changes)) for w in range(a,b+1)]
        if not vals:return []
        out=[];s0=vals[0][0];prev_w=s0;prev_t=vals[0][1]
        for w,t in vals[1:]:
            if t!=prev_t:
                out.append({"start_week":s0,"end_week":prev_w,"teacher":prev_t})
                s0=w;prev_t=t
            prev_w=w
        out.append({"start_week":s0,"end_week":prev_w,"teacher":prev_t})
        return out

    def requirements_at_week(self,week,year=None):
        year=year or self.active_year();changes=self._assignment_changes_by_req(year)
        rows=self.requirements(year)
        for r in rows:
            r["base_teacher"]=r.get("teacher") or ""
            r["teacher"]=self.effective_teacher_for_requirement(r,week,changes)
            r["view_week"]=int(week)
        return rows

    def assignment_change_notes(self,teacher,year=None,start_week=1,end_week=None):
        year=year or self.active_year();weeks=int(self.setting("weeks",35));a=int(start_week or 1);b=int(end_week or weeks)
        out=[]
        for x in self.assignment_changes(year):
            if int(x["end_week"])<a or int(x["start_week"])>b:continue
            frm=(x.get("from_teacher") or "").strip();to=(x.get("to_teacher") or "").strip()
            if teacher not in (frm,to):continue
            label=f"{x['class_name']} · {x['subject']}"+(f" · {x['req_type']}" if x.get("req_type") else "")
            if teacher==to and teacher!=frm:
                action=f"nhận {label} từ {frm or 'chưa phân công'}"
            elif teacher==frm and teacher!=to:
                action=f"giao {label} cho {to or 'chưa phân công'}"
            else:
                action=f"điều chỉnh {label}"
            wt=f"T{x['start_week']}" if int(x['start_week'])==int(x['end_week']) else f"T{x['start_week']}-T{x['end_week']}"
            note=(x.get("note") or "").strip()
            out.append(f"{wt}: {action}"+(f" ({note})" if note else ""))
        return out

    def delete_assignment_change(self,rid):
        with self.lock:
            row=self.cx.execute("SELECT * FROM assignment_changes WHERE id=?",(int(rid),)).fetchone()
            if not row:raise ValueError("Không tìm thấy biến động phân công.")
            self.cx.execute("DELETE FROM assignment_changes WHERE id=?",(int(rid),))
            self.cx.commit()
        self.invalidate_caches()
    ...
    def subject_groups(self,year=None,week=None):
        year=year or self.active_year()
        reqs=self.requirements_at_week(week,year) if week is not None else self.requirements(year)
        groups={}
        for r in reqs:
    ...
    def teacher_load_at_week(self,teacher,week,year=None,add_parts=None,exclude_ids=None,
                             ...
                             add_parts_start_week=1,add_parts_end_week=None,exclude_from_week=1,exclude_to_week=None):
        year=year or self.active_year();week=int(week)
        no_hypothesis=(not add_parts and not exclude_ids and not exclude_review_ids and not add_review_periods)
        if no_hypothesis:
            arr=self.workload_matrix(year).get(teacher)
            if arr and 1<=week<len(arr):
                return dict(arr[week])
        exclude_ids=set(exclude_ids or [])
        exclude_review_ids=set(exclude_review_ids or [])
        add_parts_start_week=int(add_parts_start_week or 1)
        add_parts_end_week=int(add_parts_end_week or self.setting("weeks",35))
        exclude_from_week=int(exclude_from_week or 1)
        exclude_to_week=int(exclude_to_week or self.setting("weeks",35))
        with self.lock:
            t=self.cx.execute("SELECT * FROM teachers WHERE year=? AND name=?",(year,teacher)).fetchone()
            if not t:return None
            reqs=[dict(r) for r in self.cx.execute("""SELECT * FROM requirements
                WHERE year=? AND start_week<=? AND end_week>=?""",(year,week,week)).fetchall()]
            changes=[dict(r) for r in self.cx.execute("SELECT * FROM assignment_changes WHERE year=? ORDER BY start_week,id",(year,)).fetchall()]
            changes_by={}
            for x in changes:changes_by.setdefault(int(x["requirement_id"]),[]).append(x)
            rules={r["subject"]:r for r in self.program_rules()}
            teaching=0.0
            for r in reqs:
                effective=self.effective_teacher_for_requirement(r,week,changes_by)
                excluded=(r["id"] in exclude_ids and exclude_from_week<=week<=exclude_to_week)
                if excluded:
                    # Một thay đổi có hiệu lực muộn hơn phải được ưu tiên hơn giả định đang kiểm tra.
                    later=[x for x in changes_by.get(int(r["id"]),[])
                           if int(x.get("start_week",1))>exclude_from_week
                           and int(x.get("start_week",1))<=week<=int(x.get("end_week",self.setting("weeks",35)))]
                    if later:excluded=False
                if effective==teacher and not excluded:
                    teaching+=self.requirement_weekly_hours(r,week,rules)
            if add_parts and add_parts_start_week<=week<=add_parts_end_week:
                for p in add_parts:
                    if not (int(p.get("start_week",1))<=week<=int(p.get("end_week",self.setting("weeks",35)))):continue
                    # Nếu đã có một biến động bắt đầu sau giả định này, biến động muộn hơn thắng.
                    later=[x for x in changes_by.get(int(p["id"]),[])
                           if int(x.get("start_week",1))>add_parts_start_week
                           and int(x.get("start_week",1))<=week<=int(x.get("end_week",self.setting("weeks",35)))]
                    if later:continue
                    if p["id"] in exclude_ids and exclude_from_week<=week<=exclude_to_week:continue
                    current_teacher=self.effective_teacher_for_requirement(p,week,changes_by)
            ...
            if add_review_periods and add_review_start is not None and int(add_review_start)<=week<=int(add_review_end or self.setting("weeks",35)):
            ...
            exam_neutral=(self.is_neutral_exam_week(week,year) and not unavailable_events
                          and int(t["active_start"])<=week<=int(t["active_end"]) and int(t["active"]))
            if exam_neutral:std=num(self.workload_policy(year).get("exam_week_standard"),17)
            total=std if exam_neutral else actual_total
            return {"teaching":teaching,"review":review,"role":role_credit,
                    ...
                    "actual_total":actual_total,"exam_neutral":exam_neutral,
                    "total":total,"standard":std,"diff":total-std if std else total}

    def _load_array_summary(self, arr, year=None):
        """Tóm tắt nhanh một ma trận tải đã có, không truy vấn DB theo từng tuần."""
        year=year or self.active_year()
        rows=[(w,arr[w]) for w in range(1,len(arr)) if arr[w] is not None]
        if not rows:
            return {"max":None,"annual":{"over_hours":0,"under_hours":0,"net_diff":0,
                    "annual_overtime_limit":num(self.workload_policy(year).get("annual_overtime_limit"),200),
                    "over_limit":False,"remaining_to_limit":num(self.workload_policy(year).get("annual_overtime_limit"),200)}}
        wmx,dmx=max(rows,key=lambda wd:(num(wd[1].get("diff")),num(wd[1].get("total"))))
        mx=dict(dmx);mx["week"]=wmx
        over=round(sum(max(num(d.get("diff")),0) for _,d in rows),6)
        under=round(sum(max(-num(d.get("diff")),0) for _,d in rows),6)
        limit=num(self.workload_policy(year).get("annual_overtime_limit"),200)
        annual={"over_hours":over,"under_hours":under,"net_diff":round(over-under,6),
                "annual_overtime_limit":limit,"over_limit":over>limit+1e-9,
                "remaining_to_limit":round(limit-over,6)}
        return {"max":mx,"annual":annual}

    def _apply_delta_to_load(self, d, teaching_delta=0.0, review_delta=0.0):
        """Áp delta lên một tuần dựa trên tải nền; tuần thi giữ total=standard."""
        x=dict(d)
        if x.get("unavailable"):
            return x
        if teaching_delta:
            x["teaching"]=max(0.0,num(x.get("teaching"))+num(teaching_delta))
        if review_delta:
            x["review"]=max(0.0,num(x.get("review"))+num(review_delta))
        delta=num(teaching_delta)+num(review_delta)
        x["actual_total"]=max(0.0,num(x.get("actual_total",x.get("total")))+delta)
        if x.get("exam_neutral"):
            x["total"]=num(x.get("standard"))
        else:
            x["total"]=max(0.0,num(x.get("total"))+delta)
        x["diff"]=num(x.get("total"))-num(x.get("standard")) if num(x.get("standard")) else num(x.get("total"))
        return x

    def simulate_assignment_loads(self, teacher, parts, start_week, end_week, mode="add", year=None):
        """Mô phỏng thêm/bớt một nhóm requirement trên cache GV×tuần, không lặp SQL 35 tuần."""
        year=year or self.active_year();weeks=int(self.setting("weeks",35))
        base=self.workload_matrix(year).get(teacher)
        if not base:return None
        out=[dict(x) if x is not None else None for x in base]
        changes_by=self._assignment_changes_by_req(year)
        sw=max(1,int(start_week or 1));ew=min(weeks,int(end_week or weeks))
        for w in range(sw,ew+1):
            delta=0.0
            for p in parts:
                if not (int(p.get("start_week",1))<=w<=int(p.get("end_week",weeks))):continue
                # Biến động có hiệu lực sau thời điểm giả định sẽ được ưu tiên.
                later=[x for x in changes_by.get(int(p["id"]),[])
                       if int(x.get("start_week",1))>sw
                       and int(x.get("start_week",1))<=w<=int(x.get("end_week",weeks))]
                if later:continue
                eff=self.effective_teacher_for_requirement(p,w,changes_by)
                h=self.requirement_weekly_hours(p,w)
                if mode=="add":
                    if eff!=teacher:delta+=h
                else:
                    if eff==teacher:delta-=h
            if delta and out[w] is not None:
                out[w]=self._apply_delta_to_load(out[w],teaching_delta=delta)
        return out

    def simulate_review_loads(self, teacher, periods, start_week, end_week, exclude_review_id=None, year=None):
        """Mô phỏng ôn TN bằng ma trận cache; hỗ trợ sửa một dòng ôn hiện hữu."""
        year=year or self.active_year();weeks=int(self.setting("weeks",35))
        base=self.workload_matrix(year).get(teacher)
        if not base:return None
        out=[dict(x) if x is not None else None for x in base]
        if exclude_review_id not in (None,"",0,"0"):
            rid=int(exclude_review_id)
            job=next((r for r in self.review_jobs(year) if int(r.get("id",0))==rid),None)
            if job and job.get("teacher")==teacher and job.get("start_week") is not None:
                a=max(1,int(job.get("start_week")));b=min(weeks,int(job.get("end_week",weeks)))
                for w in range(a,b+1):
                    if out[w] is not None:
                        out[w]=self._apply_delta_to_load(out[w],review_delta=-num(job.get("periods_per_week")))
        a=max(1,int(start_week or 1));b=min(weeks,int(end_week or weeks))
        for w in range(a,b+1):
            if out[w] is not None:
                out[w]=self._apply_delta_to_load(out[w],review_delta=num(periods))
        return out
    ...
    def teacher_max_load(self,teacher,year=None,add_parts=None,exclude_ids=None,
        ...
        for w in range(1,int(self.setting("weeks",35))+1):
            d=self.teacher_load_at_week(
                ...
                add_parts_start_week,add_parts_end_week,exclude_from_week,exclude_to_week
            )
    ...
    def teacher_overtime_projection(self,teacher,year=None,add_parts=None,exclude_ids=None,
                                    exclude_review_ids=None,add_review_periods=0,add_review_start=None,add_review_end=None,
                                    add_parts_start_week=1,add_parts_end_week=None,exclude_from_week=1,exclude_to_week=None):
        """Tính dư dương cộng dồn cho cả năm, kể cả phương án giả định trước khi gán."""
        year=year or self.active_year();weeks=int(self.setting("weeks",35))
        if not add_parts and not exclude_ids and not exclude_review_ids and not add_review_periods:
            arr=self.workload_matrix(year).get(teacher) or []
            summary=self._load_array_summary(arr,year)["annual"]
            return summary
        over=under=0.0
        for w in range(1,weeks+1):
            d=self.teacher_load_at_week(teacher,w,year,add_parts,exclude_ids,exclude_review_ids,
                                        add_review_periods,add_review_start,add_review_end,
                                        add_parts_start_week,add_parts_end_week,exclude_from_week,exclude_to_week)
            if not d:continue
            diff=num(d.get("diff"))
            over+=max(diff,0);under+=max(-diff,0)
        over=round(over,6);under=round(under,6);limit=num(self.workload_policy(year).get("annual_overtime_limit"),200)
        return {"over_hours":over,"under_hours":under,"net_diff":round(over-under,6),
                "annual_overtime_limit":limit,"over_limit":over>limit+1e-9,
                "remaining_to_limit":round(limit-over,6)}
    ...
    def workload_segments(self,teacher_name,start_week=1,end_week=None,year=None):
        """Nén tải theo các giai đoạn và tính dư/thiếu cộng dồn của từng giai đoạn."""
        year=year or self.active_year();weeks=int(self.setting("weeks",35))
        a=max(1,int(start_week or 1));b=min(weeks,int(end_week or weeks))
        if a>b:a,b=b,a
        arr=self.workload_matrix(year).get(teacher_name) or []
        vals=[]
        for w in range(a,b+1):
            d=arr[w] if w<len(arr) else None
            if not d:continue
            diff=round(num(d.get("diff")),6)
            if abs(diff)<1e-6:diff=0.0
            vals.append((w,d,diff))
        if not vals:return []
        segments=[]
        s0=vals[0][0];prev_w=s0;prev_d=vals[0][1];prev_diff=vals[0][2]
        def key(d,diff):
            return (round(num(d.get("total")),6),round(num(d.get("standard")),6),diff,bool(d.get("exam_neutral")),bool(d.get("unavailable")))
        prev_key=key(prev_d,prev_diff)
        for w,d,diff in vals[1:]:
            k=key(d,diff)
            if k!=prev_key:
                n=prev_w-s0+1
                segments.append({"start_week":s0,"end_week":prev_w,"weeks":n,
                    "total":num(prev_d.get("total")),"actual_total":num(prev_d.get("actual_total",prev_d.get("total"))),
                    "standard":num(prev_d.get("standard")),"weekly_diff":prev_diff,
                    "over_hours":round(max(prev_diff,0)*n,6),"under_hours":round(max(-prev_diff,0)*n,6),
                    "net_diff":round(prev_diff*n,6),"exam_neutral":bool(prev_d.get("exam_neutral")),
                    "unavailable":bool(prev_d.get("unavailable"))})
                s0=w;prev_key=k
            prev_w=w;prev_d=d;prev_diff=diff
        n=prev_w-s0+1
        segments.append({"start_week":s0,"end_week":prev_w,"weeks":n,
            "total":num(prev_d.get("total")),"actual_total":num(prev_d.get("actual_total",prev_d.get("total"))),
            "standard":num(prev_d.get("standard")),"weekly_diff":prev_diff,
    ...
    def _workload_segments_note(self,segments):
        parts=[]
        for x in segments:
            a=x["start_week"];b=x["end_week"];wt=f"Tuần {a}" if a==b else f"Tuần {a}-{b}"
            if x.get("exam_neutral"):
                parts.append(f"{wt} tuần thi HK: tính đủ chuẩn {self._fmt_hours_vi(x['standard'])} tiết, không phát sinh dư/thiếu")
            elif x.get("unavailable"):
                parts.append(f"{wt} không khả dụng: thiếu {self._fmt_hours_vi(x['under_hours'])} tiết trong giai đoạn")
            elif x["weekly_diff"]>0:
                parts.append(f"{wt} dư {self._fmt_hours_vi(x['weekly_diff'])} tiết/tuần → dư giai đoạn {self._fmt_hours_vi(x['over_hours'])} tiết")
            elif x["weekly_diff"]<0:
                parts.append(f"{wt} thiếu {self._fmt_hours_vi(abs(x['weekly_diff']))} tiết/tuần → thiếu giai đoạn {self._fmt_hours_vi(x['under_hours'])} tiết")
            else:
                parts.append(f"{wt} đủ chuẩn")
        return "; ".join(parts)

    def workload_change_note(self, teacher_name, year=None):
        year=year or self.active_year()
        base=self._workload_segments_note(self.workload_segments(teacher_name,1,self.setting("weeks",35),year))
        changes=self.assignment_change_notes(teacher_name,year,1,self.setting("weeks",35))
        return base + ((". Biến động phân công: "+"; ".join(changes)) if changes else "")

    def workload_change_note_range(self,teacher_name,start_week,end_week,year=None):
        year=year or self.active_year()
        base=self._workload_segments_note(self.workload_segments(teacher_name,start_week,end_week,year))
        changes=self.assignment_change_notes(teacher_name,year,start_week,end_week)
        return base + ((". Biến động phân công: "+"; ".join(changes)) if changes else "")
    ...
    def teacher_period_summary(self,teacher,start_week,end_week,year=None):
        ...
        if not rows:return {"total":0,"standard":0,"diff":0,"net_diff":0,"over_hours":0,"under_hours":0,"average":0,"average_standard":0,"max":None,"weeks":0,"segments":[],"neutral_weeks":0}
        ...
        total=round(sum(num(x["total"]) for x in rows),6);actual_total=round(sum(num(x.get("actual_total",x["total"])) for x in rows),6)
        standard=round(sum(num(x["standard"]) for x in rows),6)
        over_hours=round(sum(max(num(x["diff"]),0) for x in rows),6)
        under_hours=round(sum(max(-num(x["diff"]),0) for x in rows),6)
        net_diff=round(over_hours-under_hours,6)
        mx=max(rows,key=lambda x:(num(x["diff"]),num(x["total"])))
        segments=self.workload_segments(teacher,a,b,year)
        policy=self.workload_policy(year);limit=num(policy.get("annual_overtime_limit"),200)
        return {"teaching":teaching,"review":review,"homeroom":homeroom,"role":role,
                "total":total,"actual_total":actual_total,"standard":standard,"diff":net_diff,"net_diff":net_diff,
                "over_hours":over_hours,"under_hours":under_hours,
                "average":round(total/len(rows),6),"average_standard":round(standard/len(rows),6),
                "max":mx,"weeks":len(rows),"segments":segments,
                "neutral_weeks":sum(1 for x in rows if x.get("exam_neutral")),
                "annual_overtime_limit":limit,"over_limit":over_hours>limit+1e-9,
                "remaining_to_limit":round(limit-over_hours,6),
                "note":self._workload_segments_note(segments)}

    def teacher_annual_summary(self,teacher,year=None):
        year=year or self.active_year()
        return self.teacher_period_summary(teacher,1,int(self.setting("weeks",35)),year)
    ...
    def preview_drop(self,class_name,subject,teacher,req_type=None,week=1,effective_week=None,effective_end_week=None):
        parts=self._target_parts(class_name,subject,req_type)
        if not parts:raise ValueError("Không tìm thấy thẻ môn.")
        year=self.active_year();weeks=int(self.setting("weeks",35));week=int(week)
        effective_week=max(1,min(weeks,int(effective_week or week or 1)))
        max_part_end=max(int(p.get("end_week",weeks)) for p in parts)
        effective_end_week=max(effective_week,min(max_part_end,int(effective_end_week or max_part_end)))
        with self.lock:
            t=self.cx.execute("SELECT * FROM teachers WHERE year=? AND name=?",(year,teacher)).fetchone()
        ...
        blocking=self.blocking_events(teacher,effective_week,effective_end_week)
        covers=int(t["active_start"])<=effective_week and int(t["active_end"])>=effective_end_week and not blocking
        base=self.workload_matrix(year).get(teacher)
        if not base:raise ValueError("Không tìm thấy tải giáo viên.")
        current=dict(base[week])
        sim=self.simulate_assignment_loads(teacher,parts,effective_week,effective_end_week,"add",year)
        after=dict(sim[week])
        ss=self._load_array_summary(sim,year)
        annual_before=self._load_array_summary(base,year)["annual"]
        changes_by=self._assignment_changes_by_req(year)
        old_at_effect=[self.effective_teacher_for_requirement(p,effective_week,changes_by) for p in parts]
        added_weekly=0.0
        if effective_week<=week<=effective_end_week:
            for p,old in zip(parts,old_at_effect):
                if old!=teacher:added_weekly+=self.requirement_weekly_hours(p,week)
        return {
            ...
            "current":current,"after":after,"max_after":ss["max"],
            "annual_before":annual_before,"annual_after":ss["annual"],
            "effective_week":effective_week,"effective_end_week":effective_end_week,
            "added_weekly":added_weekly,
            "parts":[{"id":p["id"],"req_type":p["req_type"],"annual_hours":p["annual_hours"],
                      "old_teacher":old} for p,old in zip(parts,old_at_effect)]
        }

    def assign_drop(self,class_name,subject,teacher,req_type=None,effective_week=1,effective_end_week=None,note=""):
        parts=self._target_parts(class_name,subject,req_type)
        ...
        weeks=int(self.setting("weeks",35));sw=max(1,min(weeks,int(effective_week or 1)))
        ew=max(sw,min(weeks,int(effective_end_week or weeks)))
        teacher=(teacher or "").strip();now=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        year=self.active_year()
        with self.lock:
            # Luôn ghi nhận trạng thái trước khi thay đổi để có thể kiểm tra sau khi lưu.
            changes_before=self._assignment_changes_by_req(year)
            expected=[]
            for p in parts:
                before_effect=self.effective_teacher_for_requirement(p,sw-1,changes_before) if sw>int(p.get("start_week",1)) else ""
                at_effect=self.effective_teacher_for_requirement(p,sw,changes_before)
                expected.append({"requirement_id":int(p["id"]),"expected_before":before_effect,"old_at_effect":at_effect})

            if sw<=1:
                for p in parts:
                    self.cx.execute("UPDATE requirements SET teacher=? WHERE id=?",(teacher,p["id"]))
                self.cx.commit()
            else:
                created=[]
                for p,exp in zip(parts,expected):
                    old=exp["old_at_effect"]
                    auto_note=(note or "").strip() or f"Chuyển phân công từ T{sw}: {old or 'chưa phân công'} → {teacher or 'chưa phân công'}"
                    self.cx.execute("""INSERT INTO assignment_changes(
                        year,requirement_id,class_name,subject,req_type,from_teacher,to_teacher,start_week,end_week,note,created_at
                    ) VALUES(?,?,?,?,?,?,?,?,?,?,?)
                    ON CONFLICT(year,requirement_id,start_week) DO UPDATE SET
                        class_name=excluded.class_name,subject=excluded.subject,req_type=excluded.req_type,
                        from_teacher=excluded.from_teacher,to_teacher=excluded.to_teacher,end_week=excluded.end_week,
                        note=excluded.note,created_at=excluded.created_at""",
                        (year,p["id"],p["class_name"],p["subject"],p["req_type"],old,teacher,sw,
                         min(ew,int(p.get("end_week",weeks))),auto_note,now))
                    created.append({"requirement_id":p["id"],"from_teacher":old,"to_teacher":teacher,
                                    "start_week":sw,"end_week":min(ew,int(p.get("end_week",weeks)))})
                self.cx.commit()

        self.invalidate_caches()
        # Đọc lại từ DB để xác nhận: tuần trước phải giữ nguyên, tuần hiệu lực phải là GV mới.
        fresh_parts={int(x["id"]):x for x in self._target_parts(class_name,subject,req_type)}
        fresh_changes=self._assignment_changes_by_req(year)
        verification=[]
        for exp in expected:
            rp=fresh_parts.get(exp["requirement_id"])
            if not rp:continue
            before=self.effective_teacher_for_requirement(rp,sw-1,fresh_changes) if sw>int(rp.get("start_week",1)) else ""
            at=self.effective_teacher_for_requirement(rp,sw,fresh_changes)
            verification.append({"requirement_id":exp["requirement_id"],"expected_before":exp["expected_before"],
                                 "before_effect":before,"at_effect":at})
        if any(v["at_effect"]!=teacher for v in verification):
            raise ValueError("Không xác nhận được phân công tại tuần có hiệu lực. Dữ liệu chưa đạt trạng thái mong muốn.")
        if sw>1 and any(v["before_effect"]!=v["expected_before"] for v in verification):
            raise ValueError("Phân công tuần trước bị thay đổi ngoài ý muốn. Hãy kiểm tra lịch sử biến động.")
        if sw<=1:
            return {"mode":"base","effective_week":1,"end_week":ew,"verification":verification}
        return {"mode":"change","effective_week":sw,"end_week":ew,"changes":created,"verification":verification}
    ...
    def preview_review(self,teacher,periods_per_week,start_week=None,end_week=None,rid=None,view_week=1):
        year=self.active_year();periods=num(periods_per_week)
        if periods<=0:raise ValueError("Số tiết ôn phải lớn hơn 0.")
        weeks=int(self.setting("weeks",35))
        sw=None if start_week in(None,"","0",0) else int(start_week);ew=int(end_week or weeks)
        if sw is not None and not(1<=sw<=ew<=weeks):raise ValueError("Tuần bắt đầu/kết thúc không hợp lệ.")
        hypothetical_start=int(view_week) if sw is None else sw
        # Nền hiện tại sau khi bỏ dòng đang sửa (nếu có), sau đó cộng phương án mới.
        base=self.workload_matrix(year).get(teacher)
        if not base:raise ValueError("Không tìm thấy giáo viên.")
        clean=[dict(x) if x is not None else None for x in base]
        if rid not in(None,"","0",0):
            job=next((r for r in self.review_jobs(year) if int(r.get("id",0))==int(rid)),None)
            if job and job.get("teacher")==teacher and job.get("start_week") is not None:
                a=max(1,int(job.get("start_week")));b=min(weeks,int(job.get("end_week",weeks)))
                for w in range(a,b+1):
                    if clean[w] is not None:clean[w]=self._apply_delta_to_load(clean[w],review_delta=-num(job.get("periods_per_week")))
        sim=[dict(x) if x is not None else None for x in clean]
        for w in range(hypothetical_start,ew+1):
            if sim[w] is not None:sim[w]=self._apply_delta_to_load(sim[w],review_delta=periods)
        ss=self._load_array_summary(sim,year)
        current=dict(clean[int(view_week)]) if clean[int(view_week)] is not None else None
        at_start=dict(sim[hypothetical_start]) if sim[hypothetical_start] is not None else None
        return {"teacher":teacher,"periods_per_week":periods,"start_week":sw,"end_week":ew,
                "view_week":int(view_week),"current":current,"after_start":at_start,"max_after":ss["max"],
                "annual_after":ss["annual"],"hypothetical":sw is None}
    ...
    def save_review(self,d):
        ...
        # Chỉ cho phép chọn lớp 12 mà chính giáo viên phụ trách môn ôn trong toàn giai đoạn ôn.
        check_start=int(sw or 1);changes_by=self._assignment_changes_by_req(year);eligible=set()
        for r in self.requirements(year):
            if r["subject"]!=subject or not str(r["class_name"]).startswith("12"):continue
            a=max(check_start,int(r.get("start_week",1)));b=min(ew,int(r.get("end_week",self.setting("weeks",35))))
            if a<=b and all(self.effective_teacher_for_requirement(r,w,changes_by)==teacher for w in range(a,b+1)):
                eligible.add(r["class_name"])
    ...
    def staffing_summary(self,year=None):
        ...
        ratio=num(self.setting("teacher_per_class",1.8),1.8)
        return {"class_count":len(classes),"teacher_per_class":ratio,"legal_teacher_equiv":len(classes)*ratio,
...
def make_xlsx(year, week=1, scope="all", start_week=None, end_week=None):
    ...
    week=reference_week
    staff=STORE.teachers(year);classes=STORE.classes(year);reqs=STORE.requirements_at_week(week,year)
    ...
    overview=[["Năm học",year],["Phạm vi xuất",range_label],["Từ tuần",range_start],["Đến tuần",range_end],
              ...
              ["Định mức GDTX "+str(summary.get("teacher_per_class",1.8)).replace(".",",")+" GV/lớp",summary["legal_teacher_equiv"]],
              ["GV hoạt động",summary["active_teacher_count"]],["Số tuần năm học",weeks],
              ["Tuần tham chiếu trong phạm vi",week]]

    policy=STORE.workload_policy(year);annual_limit=num(policy.get("annual_overtime_limit"),200)
    staff_rows=[["STT","Họ tên","Chuyên môn chính","Có thể dạy","Các lớp dạy",f"Chuẩn T{week}",f"Tổng tiết T{week}",f"Dư giờ T{week}",f"Cao nhất {range_label}","Tuần cao nhất","Dư cộng dồn phạm vi","Thiếu cộng dồn phạm vi","Chênh lệch ròng","Dư cộng dồn cả năm","Ngưỡng dư/năm","Cảnh báo ngưỡng","Loại GV","Hiệu lực từ","Đến","Chức vụ/KN","Tiết CV/KN","Chủ nhiệm","Tiết CN","Trạng thái","Ghi chú"]]
    for t in staff:
        ...
        annual_summary=STORE.teacher_annual_summary(t["name"],year)
        ...
        staff_rows.append([t["stt"],t["name"],t["primary_subject"],", ".join(t["can_teach_list"]),
                           ...
                           period_summary["over_hours"],period_summary["under_hours"],period_summary["net_diff"],
                           annual_summary["over_hours"],annual_limit,"VƯỢT" if annual_summary["over_limit"] else "OK",
    ...
    period_rows=[["STT","Họ tên","Chuyên môn","Phạm vi","Số tuần","Tiết dạy","Ôn TN","Chủ nhiệm","Chức vụ/KN","Tổng tiết phạm vi","Chuẩn phạm vi","Dư cộng dồn","Thiếu cộng dồn","Chênh lệch ròng","BQ tiết/tuần","Cao nhất tuần","Tuần cao nhất","Tuần thi trung hòa","Ghi chú biến động"]]
    for t in staff:
        ...
        period_rows.append([t["stt"],t["name"],t["primary_subject"],range_label,ps["weeks"],ps["teaching"],ps["review"],ps["homeroom"],ps["role"],
                            ps["total"],ps["standard"],ps["over_hours"],ps["under_hours"],ps["net_diff"],ps["average"],mx.get("total",0),mx.get("week",""),ps.get("neutral_weeks",0),ps.get("note","")])

    segment_rows=[["STT","Họ tên","Chuyên môn","Từ tuần","Đến tuần","Số tuần","Tổng/tuần","Chuẩn/tuần","Dư-thiếu/tuần","Dư giai đoạn","Thiếu giai đoạn","Chênh lệch giai đoạn","Tuần thi HK","Ghi chú"]]
    for t in staff:
        for seg in STORE.workload_segments(t["name"],range_start,range_end,year):
            note=(f"Tuần thi HK: tính đủ chuẩn {STORE._fmt_hours_vi(seg['standard'])} tiết" if seg.get("exam_neutral") else
                  "Không khả dụng" if seg.get("unavailable") else "")
            segment_rows.append([t["stt"],t["name"],t["primary_subject"],seg["start_week"],seg["end_week"],seg["weeks"],
                                 seg["total"],seg["standard"],seg["weekly_diff"],seg["over_hours"],seg["under_hours"],seg["net_diff"],
                                 "Có" if seg.get("exam_neutral") else "",note])
    ...
    assignment_change_rows=[["Lớp","Môn","Loại","GV giao","GV nhận","Có hiệu lực từ tuần","Đến tuần","Ghi chú","Thời điểm tạo"]]
    for x in STORE.assignment_changes(year):
        if not _range_overlap(x.get("start_week"),x.get("end_week"),range_start,range_end):continue
        assignment_change_rows.append([x["class_name"],x["subject"],x["req_type"],x["from_teacher"],x["to_teacher"],
                                       x["start_week"],x["end_week"],x["note"],x.get("created_at","")])
    ...
    sheets=[
        ...
        ("Doi ngu",staff_rows,[6,24,16,28,50,9,11,11,13,11,14,14,14,16,12,14,14,10,8,26,10,16,10,12,70]),
        ("Tong hop pham vi",period_rows,[6,24,16,18,10,12,10,12,14,18,16,14,14,16,14,14,12,14,72]),
        ("Giai doan du gio",segment_rows,[6,24,16,10,10,10,12,12,14,14,14,16,12,42]),
        ("Lich su CN-KN",duty_rows,[24,14,28,10,12,14,10,10,32]),
        ("Bien dong nhan su",event_rows,[24,20,10,10,10,24,36]),
        ("Bien dong phan cong",assignment_change_rows,[10,18,14,24,24,16,10,48,20]),
...
def make_report_html(week=1,scope="all",start_week=None,end_week=None):
    ...
    for t in STORE.teachers(active_only=True):
        ...
        annual=STORE.teacher_annual_summary(t["name"],year)
        rows.append(f"<tr><td>{escape(t['name'])}</td><td>{escape(t['primary_subject'])}</td>"
                    ...
                    f"<td>{STORE._fmt_hours_vi(ps['over_hours'])}</td><td>{STORE._fmt_hours_vi(ps['under_hours'])}</td><td>{STORE._fmt_hours_vi(ps['net_diff'])}</td>"
                    f"<td>{STORE._fmt_hours_vi(annual['over_hours'])}/{STORE._fmt_hours_vi(annual['annual_overtime_limit'])}{' ⚠' if annual['over_limit'] else ''}</td>"
                    f"<td>T{mx.get('week','')} · {STORE._fmt_hours_vi(mx.get('total',0))}</td></tr>")
    ...
    <table><thead><tr><th>Giáo viên</th><th>Chuyên môn</th><th>Chuẩn phạm vi</th><th>Tổng tiết phạm vi</th><th>Dư cộng dồn</th><th>Thiếu cộng dồn</th><th>Chênh lệch ròng</th><th>Dư cả năm/ngưỡng</th><th>Cao nhất phạm vi</th></tr></thead>
...
def make_report_docx(week=1,scope="all",start_week=None,end_week=None):
    ...
    headers=["Giáo viên","Chuyên môn","Chuẩn phạm vi","Tổng phạm vi","Dư cộng dồn","Thiếu cộng dồn","Chênh lệch ròng","Dư năm/ngưỡng","Cao nhất"]
    table_rows=[headers]
    for t in STORE.teachers(active_only=True):
        ps=STORE.teacher_period_summary(t["name"],a,b);mx=ps.get("max") or {}
        annual=STORE.teacher_annual_summary(t["name"],year)
        table_rows.append([t["name"],t["primary_subject"],STORE._fmt_hours_vi(ps["standard"]),
                           STORE._fmt_hours_vi(ps["total"]),STORE._fmt_hours_vi(ps["over_hours"]),STORE._fmt_hours_vi(ps["under_hours"]),STORE._fmt_hours_vi(ps["net_diff"]),
                           f'{STORE._fmt_hours_vi(annual["over_hours"])}/{STORE._fmt_hours_vi(annual["annual_overtime_limit"])}'+(" !" if annual["over_limit"] else ""),
...
def dashboard_data(week=1):
    ...
    reqs=STORE.requirements_at_week(week,year)
...
def mutation_label(path,d):
    labels={
        ...
        "/api/assignment_change/delete":"Xóa biến động phân công",
    ...
    extra=""
    if path=="/api/drop":
        extra=f' {d.get("class_name","")} {d.get("subject","")} → {d.get("teacher","") or "Bỏ gán"} · hiệu lực T{d.get("effective_week",1)}'
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if u.path=="/api/version":
                self.send_json({"version":"10.3.8","build":"lazy-pages-fast-sim","index":os.path.basename(INDEX_PATH),"port":self.server.server_address[1]});return
            if u.path=="/":
            ...
            if u.path=="/api/state":
                ...
                # Base state nhẹ: chỉ dữ liệu dùng chung. Dữ liệu nặng được tải theo trang.
                self.send_json({
                    "active_year":STORE.active_year(),"years":STORE.years(),
                    "settings":{"weeks":STORE.setting("weeks",35),
                                "teacher_per_class":STORE.setting("teacher_per_class",1.8),
                                ...
                                "workload_policy":STORE.workload_policy(),
                                "sources":STORE.setting("sources",{})},
                    "teachers":STORE.teachers(),"classes":STORE.classes(),
                    "templates":STORE.templates(),"program_rules":STORE.program_rules(),
                    "duties":STORE.duties(),"staff_events":STORE.staff_events(),
                    "active_scenario":STORE.active_scenario_row(),
                    "_perf":{"mode":"base","week":week}
                });return
            if u.path=="/api/page/assign":
                q=urllib.parse.parse_qs(u.query);week=max(1,min(int(STORE.setting("weeks",35)),int((q.get("week") or ["1"])[0])))
                year=STORE.active_year()
                self.send_json({"groups":STORE.subject_groups(year,week),"requirements":STORE.requirements_at_week(week,year),
                                "assignment_changes":STORE.assignment_changes(year),"locks":STORE.locks(),
                                "loads":STORE.teacher_loads_summary(week)});return
            if u.path=="/api/page/staff":
                q=urllib.parse.parse_qs(u.query);week=max(1,min(int(STORE.setting("weeks",35)),int((q.get("week") or ["1"])[0])))
                year=STORE.active_year();teachers=STORE.teachers()
                self.send_json({"requirements":STORE.requirements_at_week(week,year),"loads":STORE.teacher_loads_summary(week),
                                "workload_notes":{t["name"]:STORE.workload_change_note(t["name"],year) for t in teachers},
                                "annual_workloads":{t["name"]:STORE.teacher_annual_summary(t["name"],year) for t in teachers}});return
            if u.path=="/api/page/review":
                q=urllib.parse.parse_qs(u.query);week=max(1,min(int(STORE.setting("weeks",35)),int((q.get("week") or ["1"])[0])))
                year=STORE.active_year();jobs=STORE.review_jobs(year)
                self.send_json({"requirements":STORE.requirements_at_week(week,year),"review_jobs":jobs,
                                "review_previews":{str(r["id"]):STORE.preview_review(r["teacher"],r["periods_per_week"],r["start_week"],r["end_week"],r["id"],week) for r in jobs}});return
            if u.path=="/api/page/timetable":
                self.send_json({"timetable_entries":STORE.timetable_entries()});return
            if u.path=="/api/page/staffing":
                self.send_json({"staffing":STORE.staffing_summary()});return
            if u.path=="/api/preview_drop":
                q=urllib.parse.parse_qs(u.query)
                w=int((q.get("week") or ["1"])[0]);eff=int((q.get("effective_week") or [str(w)])[0])
    ...
    def do_POST(self):
        try:
            ...
            if p=="/api/settings/workload_policy":
                self.send_json({"ok":True,"policy":STORE.save_workload_policy(d)});return
            ...
            auditable={
                ...
                "/api/staff_event/save","/api/staff_event/delete","/api/assignment_change/delete",
            ...
            if p=="/api/drop":
                if "effective_week" not in d or d.get("effective_week") in (None,""):
                    raise ValueError("Phải chọn rõ 'Có hiệu lực từ tuần'. Hệ thống không tự mặc định T1.")
                result["assignment"]=STORE.assign_drop(d["class_name"],d["subject"],d.get("teacher",""),d.get("req_type") or None,
                                                       d.get("effective_week"),d.get("effective_end_week"),d.get("note",""))
            ...
            elif p=="/api/assignment_change/delete":STORE.delete_assignment_change(int(d["id"]))
...
def main():
    server=None;chosen_port=None
    last_error=None
    for port in range(PORT,PORT_FALLBACK_MAX+1):
        try:
            ...
            last_error=e
    if server is None:
        raise RuntimeError(f"Không tìm được cổng trống từ {PORT} đến {PORT_FALLBACK_MAX}: {last_error}")
    url=f"http://{HOST}:{chosen_port}/?v=10.3.8"
    print("="*72);print("PHẦN MỀM PHÂN CÔNG V10.3.8 — KHỞI ĐỘNG TÁCH BIỆT · HIỆU LỰC THEO TUẦN · GDTX 1,8 GV/LỚP")
    if chosen_port!=PORT:
        print(f"LƯU Ý: Cổng {PORT} đang được bản khác sử dụng → bản V10.3.8 tự mở ở cổng {chosen_port}.")
    print("Mở:",url)
